...
La transformación prioritaria de plataforma no es solo un reemplazo puntual de APIM por fin de vida del producto, sino la evolución desde un esquema monolítico hacia una arquitectura multicluster con gobierno central y operación desacoplada por dominios [2](https://github.bancogalicia.com.ar/ocpa/apim-doc/blob/master/02_multi-cluster/vision_estrategia_multicluster.md), [26](https://github.bancogalicia.com.ar/ocpa/apim-doc/blob/master/02_multi-cluster/06_alternativas_tecnologicas_evaluadas/6.2_alternativas_de_api_management_y_api_gateway.md). Nuestro API Manager se utiliza como caso modelador para validar decisiones de red, seguridad, resiliencia y operación, que luego se extienden al resto de OpenShift.

La situación actual representa un alto riesgo sistémico: un cluster productivo de gran escala soportando cargas críticas de múltiples líneas de negocio, con alto volumen transaccional y fuerte dependencia de procesos manuales para red, DNS, certificados, sincronización intersite y continuidad [11](https://github.bancogalicia.com.ar/ocpa/apim-doc/blob/master/02_multi-cluster/04_diagnostico_problemas_deuda_tecnica/4.1_riesgo_sistemico_y_blast_radius.md). Este diseño amplifica blast radius, extiende ventanas de mantenimiento y limita la elasticidad real.

La estrategia objetivo define una colección de clusters con responsabilidades claras, segmentación por criticidad y tipo de servicio, patrones diferenciados para tráfico north-south y east-west, y un modelo operativo GitOps + IaC para ciclo day 0/day 1/day 2 [40](https://github.bancogalicia.com.ar/ocpa/apim-doc/blob/master/02_multi-cluster/07_arquitectura_objetivo_plataforma/7.7_modelo_operativo_gitops_iac.md), [34](https://github.bancogalicia.com.ar/ocpa/apim-doc/blob/master/02_multi-cluster/07_arquitectura_objetivo_plataforma/7.1_modelo_multicluster_objetivo_y_segmentacion_de_dominios.md). El beneficio general esperado es reducir el impacto cruzado, mejorar continuidad operativa y sostener crecimiento sin incrementar proporcionalmente la complejidad.
...
![Pilares de limitaciones técnicas — Modelo operativo, Escalabilidad, Blast radius, Life cycle](propuesta-implementacion-ocp-imagenes/diagrama-pilares-modelo-operativo-escalabilidad-blast-lifecycle.png)
...
- **Flujo resumido:** Internet / Core / Legacy / DMZ → API Manager → Ingress Routing → Service Mesh → Backend Services. Coexistencia temporal con el modelo actual (RH 3Scale) durante la transición; decisión pendiente de vendor para API Manager (p. ej. Gloo, Kong) por dominio/fase según [01_apim — Arquitectura objetivo](https://github.bancogalicia.com.ar/ocpa/apim-doc/blob/master/01_apim/05_arquitectura_objetivo.md).
- **Despliegue del Cilium Envoy L7 gateway:** modelo on-demand por namespace cuando se requieran capacidades L7 avanzadas (rate limits, circuit breakers, canary, traffic splitting), con configuración declarativa y GitOps [01_apim — Decisiones técnicas](https://github.bancogalicia.com.ar/ocpa/apim-doc/blob/master/01_apim/04_decisiones_tecnicas.md).
...
- **Stack y responsabilidades:** Service mesh sidecarless (Cilium Mesh Enterprise) para east-west multicluster; plataforma provee L4 (conectividad, mTLS, service discovery), DevOps instrumenta L7 avanzado on-demand a traves de Cilium Envoy L7 gateway. Gobierno de APIs internas mediante políticas nativas del mesh (identidades Kubernetes, p. ej. service accounts) en lugar de API keys estáticas; ver [01_apim — Arquitectura objetivo](https://github.bancogalicia.com.ar/ocpa/apim-doc/blob/master/01_apim/05_arquitectura_objetivo.md) y [decisiones técnicas](https://github.bancogalicia.com.ar/ocpa/apim-doc/blob/master/01_apim/04_decisiones_tecnicas.md).
...
- Separar puntos de ingreso por función (gestión OCP, rutas actuales, gateway interno, API management externo).
...
La asignación de workloads a clusters y la priorización por oleadas se rige por los criterios de elegibilidad y go/no-go definidos en el programa ([sección 4](#sec-4), tabla de decisiones; [sección 5.4](#sec-5)).
